...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        self.subs = {
            "1": [["全部", "1"], ["动作片", "5"], ["喜剧片", "10"], ["科幻片", "7"], ["恐怖片", "8"], ["战争片", "9"], ["动画片", "41"], ["剧情片", "12"], ["爱情片", "6"], ["纪录片", "11"]],
    ...
    def _get(self, path):
        ...
        try:
            ...
            if r.status_code >= 400:
                print("[WARN] status=%s url=%s" % (r.status_code, url))
            return r.text
        except requests.exceptions.Timeout:
            print("[ERROR] 请求超时: %s" % url)
        except requests.exceptions.ConnectionError:
            print("[ERROR] 连接错误: %s" % url)
        except Exception as e:
            print("[ERROR] 请求失败: %s, %s" % (url, str(e)))
        return None

    def _post(self, path, data):
        try:
            r = requests.post(self.host + path, data=data, headers=self.headers, timeout=15)
            r.encoding = "utf-8"
            return r.text
        except Exception as e:
            print("[ERROR] POST失败: %s, %s" % (path, str(e)))
        return None
    ...
    def _parse_list(self, html):
        ...
        out, seen = [], set()
        # 通用正则兜底，lxml可用也做双重保险
        for vid, title, pic in re.findall(r'href="[^"]*?/movie/index(\d+)\.html"[^>]*?title="([^"]*)".*?data-original="([^"]+)"', html):
            if vid in seen:
                continue
            seen.add(vid)
            out.append({"vod_id": vid, "vod_name": title, "vod_pic": self._fix(pic)})
        # lxml解析优先
        if etree is not None:
            tree = etree.HTML(html)
            items = tree.xpath('//a[contains(@href,"/movie/index")]')
            for it in items:
                try:
                    ...
                    for at in ("data-original", "data-src", "data-echo", "data-lazy", "src"):
                        ...
                        if cand and "load.gif" not in cand[0] and "loading" not in cand[0]:
                            pic = cand[0]
                            break
                    note = " ".join(x.strip() for x in it.xpath('.//span//text()') if x.strip())
                    out.append({"vod_id": m.group(1), "vod_name": name, "vod_pic": self._fix(pic), "vod_remarks": note[:40]})
                except Exception:
                    continue
        # 去重
        ret = []
    ...
    def _parse_playlist(self, html, vid):
        ...
        if etree is not None:
            ...
            # lxml缺失正则解析播放列表
            group_data = {}
            for href, title in re.findall(r'<a href="/play/%s-(\d+)-(\d+)\.html"[^>]*?title="([^"]*)"'%vid, html):
                s, e = int(href[0]), int(href[1])
                nm = title.strip()
                group_data.setdefault(s, []).append((e, nm))
            for sid in sorted(group_data.keys()):
                eps = []
                for e, nm in sorted(group_data[sid], key=lambda x:x[0]):
                    eps.append(f"{nm}$/play/{vid}-{sid}-{e}.html")
                froms.append(f"线路{sid+1}")
                urls.append("#".join(eps))
        return froms, urls
    ...
    def homeVideoContent(self):
        return {"list": self._parse_list(self._get("/index.html"))}
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # 修复分页：无数据则pagecount=当前页，否则+1，盒子自动判断是否还有下一页
        if len(lst) == 0:
            pagecount = pg
        else:
            pagecount = pg + 1
        return {"page": pg, "pagecount": pagecount, "limit": 24, "total": 999999, "list": lst}

    def searchContent(self, key, quick, pg="1"):
        pg = int(pg or 1)
        html = self._get("/search.php?searchword=%s&page=%s" % (quote(key), pg))
        lst = self._parse_list(html)
        # get搜索没结果才尝试post搜索
        if not lst and pg == 1:
            html_post = self._post("/search.php", {"searchword": key, "searchtype": "1"})
            lst = self._parse_list(html_post)
    ...
    def detailContent(self, ids):
        ...
        vod_pic = self._meta(html, "og:image")
        vod_name = self._meta(html, "og:title")
        if not vod_name:
            mtitle = re.search(r'<h1[^>]*>(.*?)</h1>', html, re.S)
            vod_name = mtitle.group(1).strip() if mtitle else ""
        vod_name = vod_name.split("《")[-1].split("》")[0] or vod_name

        vod = {
            "vod_id": vid,
            "vod_name": vod_name.strip(),
            "vod_pic": self._fix(vod_pic),
            "vod_year": self._field(text_raw, "年份"),
            "vod_area": self._field(text_raw, "地区"),
            "type_name": self._field(text_raw, "类型"),
            "vod_lang": self._field(text_raw, "语言"),
            "vod_actor": self._field(text_raw, "主演"),
            "vod_director": self._field(text_raw, "导演"),
            "vod_remarks": self._field(text_raw, "豆瓣"),
            "vod_content": self._meta(html, "og:description") or self._meta(html, "description"),
        ...
        for p in rules:
            ...
            if val.startswith("{"):
                try:
                    ...
                    m2 = re.search(r'"(https?://[^"]+\.(?:m3u8|mp4)[^"]*)"', val)
                    val = m2.group(1) if m2 else ""
            if val and not val.startswith("#"):
                url = self._fix(val)
                break
        if url:
            return {"parse": 0, "url": url, "header": {"User-Agent": self.headers["User-Agent"], "Referer": pid}}
        # 提取不到真实地址，交给内置网页解析
